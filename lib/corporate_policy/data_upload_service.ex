defmodule CorporatePolicy.DataUploadService do
  ...
  def process_upload_background(
    ...
    Repo.transaction(fn ->
      case Repo.insert(MasterPolicyDataUpload.changeset(%MasterPolicyDataUpload{}, upload_attrs)) do
        {:ok, upload} ->
          # Enqueue background Oban job
          %{"upload_id" => upload.id}
          |> CorporatePolicy.Workers.CsvImportWorker.new()
          |> Oban.insert!()
  ...
  def process_file_with_progress(
    ...
    if progress_callback, do: progress_callback.(10)
    process_file("Ecards", file_path, policy_id, Path.basename(file_path), user_id, upload_id)
    if progress_callback, do: progress_callback.(100)
  end
  ...
  def process_file_with_progress(
    ...
    case data_type do
      "Inception Data" ->
        process_inception_with_progress(rows, policy_id, user_id, progress_callback)

      "Endorsement Data" ->
        process_endorsement_with_progress(rows, policy_id, user_id, upload_id, progress_callback)

      "Claim Dumps" ->
        process_claim_dumps_with_progress(rows, policy_id, progress_callback)
  ...
  defp process_inception_with_progress(rows, policy_id, user_id, progress_callback) do
    total = length(rows)

    Enum.reduce(rows, 0, fn row, idx ->
  ...
  defp process_endorsement_with_progress(rows, policy_id, user_id, upload_id, progress_callback) do
    total = length(rows)
  ...
  defp process_claim_dumps_with_progress(rows, policy_id, progress_callback) do
    total = length(rows)
