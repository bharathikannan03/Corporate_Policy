defmodule CorporatePolicyWeb.Admin.CdAccountsLiveTest do
  ...
  setup do
    ...
    corporate =
      Repo.insert!(%Corporate{
        corporate_name: "Corporate CD Live",
  ...
  test "standalone CD Accounts mount and display page", %{conn: conn, user: user} do
    conn = conn |> init_test_session(current_user_id: user.id)
    {:ok, _view, html} = live(conn, ~p"/admin/cd-statements/cd-accounts")
    assert html =~ "CD Accounts"
  end

  test "policy-specific CD Accounts form submit creates new account", %{
    conn: conn,
    user: user,
    policy: policy
  } do
    conn = conn |> init_test_session(current_user_id: user.id)
    {:ok, view, html} = live(conn, ~p"/admin/policy-details/#{policy.id}/cd-accounts/new")

    assert html =~ "CD Accounts"

    # Fill and submit form
    form_data = %{
      "cd_number" => "CD-NUM-001",
      "corporate_id" => to_string(policy.ref_corporate_id),
    ...
    assert {:error, {:live_redirect, %{to: to_path}}} =
             view
             |> form("#add-cd-account-form", form_data)
             |> render_submit()

    assert to_path =~ "/cd-statements/new"
  end
end
